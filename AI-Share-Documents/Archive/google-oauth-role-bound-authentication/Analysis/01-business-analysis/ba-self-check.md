...
## Known Gaps
- Exact manual Admin provisioning mechanism is not specified and is marked `Status: ASSUMED`.
- Exact post-authentication token/session contract is not specified and is marked `Status: ASSUMED`.
- Exact Google identity matching rule is not specified and is marked `Status: ASSUMED`.
- Exact disabled-account error response behavior is not specified and is marked `Status: ASSUMED`.
- Existing account behavior for Google login with a non-Photographer role requires downstream design refinement without violating immutability or no-Google-Admin-creation rules.
